...
- [x] 検討その２：アーキテクチャについての検討をさらに進める。
    - 検討の結果、以下の方針が決まった。
    - **課題1（Arch 系以外への対応）：**
        - コンポーネントは OS を知らない。OS 差分は `platform_*` フックに完全に委譲する。
        - フックの粒度は「ソフトウェアレベル」とする
          （例: `platform_install_p10k`）。
          「パッケージレベル」（`platform_install_pkg "zsh-theme-powerlevel10k-git"`）では
          パッケージ名が OS 間で異なる問題を吸収できないため。
        - `platforms/` はグルーピング（継承）構造を持つ。
          具体的には `_base` → `_arch_based` → `cachyos` のように、
          差分が発生した層だけ上書きする。
          Debian 系（Kali Linux など）・macOS も同じ起点（`_base`）から派生させる。
    - **課題2（ホストごとのオン・オフ）：**
        - `hosts/setup.bash` で呼ぶ関数を細かく選ぶことで実現する。
          （例: `setup_editor` を `_setup_neovim` と `_setup_vscode` に分割し、
          不要なものは `hosts/setup.bash` から呼ばない。）
        - 変数による制御や関数の上書きは採用しない
          （暗黙の依存が増え、読みにくくなるため）。
    - 残課題は「検討その３」として別タスクに切り出した（下記参照）。
- [x] 検討その３：アーキテクチャ検討その２で残った未決事項を解消する
    - 以下の3点を決定してから、実装（v4.10.0）に着手すること。
    - **①フック名の命名規則：**
        - `platform_{タイミング}_{対象}` 形式に決定（例: `platform_pre_install_p10k`）
        - 理由: 「いつ呼ばれるか」が先に来ることで、コードの読み順と呼び出し順が一致する
    - **③`0_core.bash` 相当の処理の扱い：**
        - `components/core.bash` として独立させることに決定
        - OS固有部分（pacmanミラー更新・AUR helperなど）はフック機構で吸収する
        - OS横断の処理（Git・mise・言語インストールなど）はそのまま `core.bash` に書く
    - **④移行戦略（段階的 vs 一括置き換え）：**
        - 一括置き換えに決定
...
- [x] mise global config の責務を再検討する（下準備として検討済み。ADR は全体アーキテクチャ設計後に作成する）
    - **検討結果（2026-06-11）**: **現時点では**現状維持とする。`"latest"` を使い続ける限り
      `mise use --global` は `config.toml` を実質変更しないため、差分問題は発生しない。ADR-005 で
      config.toml を source of truth にする案を意識的に不採用にしており、その決定を覆す理由もない。
      v4.10.0 のアーキテクチャ見直しの前に先行して変えると二度手間になるため、本格的な検討はそちらに委ねる。
    - **論点の整理**（v4.10.0 設計時の参考として残す）:
        - **① `mise use --global` の実行結果として config を更新する運用を続けるか**:
          `"latest"` を指定している限り、`mise use --global go@latest` を実行しても
          すでに `go = "latest"` と書いてあれば config.toml は実質変更されない。
          問題が顕在化するのはバージョンを固定したとき（例: `go = "1.24.0"` → `1.25.0` に更新）。
          今は `"latest"` を使っているため実害なし。
        - **② host ごとに別の mise config を持たせるか**:
          現在ホストは1台のみで差分もない。将来ホストが増えても Go・Node.js は共通で使う見込み。
          ただし「1マシン内で複数の開発環境を使い分ける」ユースケース（後述）を考えると、
          プロジェクトごとの mise config の方が重要になる可能性がある。
        - **③ mise config を「install 処理の source of truth」にするか、「補助設定・結果」として扱うか**:
          現状は「どちらでもない曖昧な中間」の状態。`config.toml` にも `go = "latest"` と書いてあり、
          `_install_golang()` の中の `mise use --global go@latest` にも同じ情報が存在している。
          ADR-005 で「source of truth にすると `_install_golang()` だけでは何を入れるかを読めなくなる」
          として案Aを不採用にしているが、この評価自体も全体設計の中で見直す余地がある。
    - **将来の設計方針として検討すべき方向性**:
        - マシン・プロジェクトが増えた場合、「プロジェクトごとの開発環境」は mise の
          ローカル config（`.mise.toml`）で管理し、「マシン共通のツール」だけを myenv で管理する
          という役割分担が自然な落としどころになりそう。例えば，プロジェクトごとに違うバージョンの Golang を
          使う，といったケースは非常に有り得る。
        - その場合、myenv が管理する `~/.config/mise/config.toml`（グローバル config）には
          「どのプロジェクトでも使う最低限のもの」だけを書き、プロジェクト固有のツールは
          各リポジトリの `.mise.toml` に任せる設計が考えられる。つまり，マシングローバルで使うものは
          myenv で直接管理する ＋ mise のグローバル config で管理する，という形か？
        - 「グローバル config に何を書くか」「symlink のまま管理するか」は
          全体アーキテクチャ（v4.10.0）の設計時に改めて検討・決定すること。
        - そして，全体アーキテクチャ（v4.10.0）の設計時に改めて検討した際，今回の検討内容も含めて，ADR として
          記録すること。
- [ ] mise 管理ツールのバージョン固定方針を検討する (優先度低)
    ...
    - **注意**: 「mise global config の責務」の検討結果（上記）も踏まえて判断すること。
      特にバージョン固定した場合に config.toml への差分問題が顕在化するため、
      global config の扱いと合わせて全体アーキテクチャ（v4.10.0）の設計時に検討すること。
