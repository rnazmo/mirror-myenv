...
- [ ] 検討その３：アーキテクチャ検討その２で残った未決事項を解消する
    - 以下の3点を決定してから、実装（v4.10.0）に着手すること。
    - **①フック名の命名規則：**
        - 現在の候補は `platform_{タイミング}_{対象}` 形式（例: `platform_pre_install_p10k`）
          と `platform_{対象}_{イベント}` 形式（例: `platform_shell_pre_p10k`）の2つ。
        - どちらかに統一してから実装を始めること。
        - → ディレクトリ構成や全体の依存関係などを見てから決めたいかも
    - **③`0_core.bash` 相当の処理の扱い：**
        - pacman ミラー更新・Git・AUR helper・mise・言語インストールなどは
          「機能のセットアップ」というより「セットアップ全体の前提条件」に近い。
        - `components/core.bash` として独立させるか、
          `hosts/setup.bash` 内に直接書くかを決める。
        - → こちらも，ディレクトリ構成や全体の依存関係などを見てから決めたいかも
    - **④移行戦略（段階的 vs 一括置き換え）：**
        - `recipes/` を残したまま段階的に移行するか、一括で置き換えるかを決める。
        - 段階的移行は安全だが、移行期間中に新旧が混在して混乱しやすい。
        - → これは一括で良い
